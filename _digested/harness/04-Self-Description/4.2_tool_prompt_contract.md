...
- `ToolDefinition` 有 `promptSnippet`（一行能力描述）和 `promptGuidelines`（使用准则），`extensions/types.ts:457-459`。
- 内置工具每个都挂了：`bash.ts:334`、`read.ts:219`、`edit.ts:308`、`write.ts:197`。
- 第三方扩展 `registerTool` 时同样可以带（`extensions/types.ts:457`，`examples/extensions/dynamic-tools.ts` 就是带 snippet + guidelines 的例子）。

这些字段由 `buildSystemPrompt` 聚合进 "Available tools" 和 "Guidelines"（`system-prompt.ts:80-117`，`agent-session.ts:1026-1054`）。
...
`buildSystemPrompt` 只列"提供了 snippet 的工具"（`system-prompt.ts:80-84`），一个都没提供时显示 `(none)`。测试明确验证了这点（`system-prompt.test.ts`："omits custom tools from available tools section when promptSnippet is not provided"）。
...
- `packages/coding-agent/src/core/extensions/types.ts`：`ToolDefinition.promptSnippet`/`promptGuidelines`（L457-459）
- `packages/coding-agent/src/core/tools/bash.ts`：`promptSnippet`/`promptGuidelines`（L334-335）；`read.ts:219`、`edit.ts:308`、`write.ts:197` 同构
- `packages/coding-agent/src/core/system-prompt.ts`：tools/guidelines 聚合（L80-117）——从定义读 snippet，非手写
- `packages/coding-agent/src/core/agent-session.ts`：guidelines 聚合（L1026-1054）
...
**例证 1（同源）：** 看 `bash.ts:334`——`promptSnippet` 是 `createBashToolDefinition` 返回对象的一个字段，和 `execute` 在同一个对象里。再看 `system-prompt.ts:80`——"Available tools" 从这些字段聚合。证明"自述"和"能力"同源，没有平行的第二份文本。
