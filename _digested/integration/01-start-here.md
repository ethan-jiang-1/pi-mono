...
```
pi-ai          ← 纯 LLM 抽象层。一般不需要直接集成
pi-agent       ← Agent 内核。适合只想复用循环逻辑的场景
pi-coding-agent ← 完整产品层。SDK、RPC、工具、扩展都在这里
```
...
- **RPC**：[`packages/coding-agent/src/modes/rpc/rpc-types.ts`](../../packages/coding-agent/src/modes/rpc/rpc-types.ts) — 22 个命令的 JSONL 协议。
...
pi-mono 的 web-ui（[`apps/web-ui/`](../../apps/web-ui/)）是一个**纯前端应用**。Agent 直接在浏览器里运行，直接调用 LLM provider API，中间没有 server。这和 OpenCode 的 server-client web-ui 完全不同。

如果你的产品需要一个 web 界面，有两种做法：

1. **参考 web-ui 的纯前端方案**：在浏览器运行 pi-agent，直接调 LLM provider。
...
### SDK 路线
1. `npm install pi-mono`（或从 monorepo 构建）
2. `import { createAgentSession } from "pi-mono/coding-agent"`
3. 调 `createAgentSession({ model: ... })`
4. 调 `session.prompt("hello")`
5. 监听 `session.on("event", ...)` 拿到流式事件
