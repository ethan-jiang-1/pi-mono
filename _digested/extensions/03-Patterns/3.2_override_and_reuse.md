...
4. **slot 分离**：执行和渲染是两个独立 slot（见 [2.3](../02-Expansion/2.3_presentation.md)）。
...
plan-mode 要"只读"，靠的是 `setActiveTools`（切工具集）+ `tool_call` 拦截（拦 bash），而不是覆盖 7 个工具。todo 要"记住列表"，靠的是 `details`（见 [2.4](../02-Expansion/2.4_persistence.md)）。ssh 要"远程执行"，靠的是 operations。
