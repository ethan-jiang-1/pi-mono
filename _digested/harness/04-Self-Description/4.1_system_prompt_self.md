...
pi 的 system prompt 第一句就自报家门（`system-prompt.ts:121`）：

> "You are an expert coding assistant operating inside pi, a coding agent harness."

随后是 `Available tools`（工具列表）、`Guidelines`（准则），再往后是一段**专门告诉 agent "pi 的文档在哪、什么时候读"**的指引（`system-prompt.ts:131-138`）：
...
`system-prompt.ts:121` 的第一句同时做了两件事：告诉 agent "你是 coding agent"，告诉它 "你在 pi 这个 harness 里"。**身份定位放在 prompt 最开头**，因为后面所有指令（工具、文档、项目上下文）都以"你在这个 harness 里"为前提。
...
`Available tools` 只列"提供了 snippet 的工具"（`system-prompt.ts:80-84`），guidelines 从工具的 `promptGuidelines` 聚合（`agent-session.ts:1026-1054`）。所以工具集变化时，prompt 里的工具列表和准则**自动跟着变**。
...
- skills 块只在 read 工具可用时才生成（`system-prompt.ts:155`）——不给 agent 一个"能看列表但读不了"的空承诺。
- 文档指引明确"resolve docs/... under Additional docs ... not the current working directory"（`system-prompt.ts:135`）——消除相对路径歧义。
...
- `packages/coding-agent/src/core/system-prompt.ts`：`buildSystemPrompt`（L28）、自报家门 + 文档地图（L121-138）、tools/guidelines 聚合（L80-117）、skills 闸门（L155）、custom prompt 分支（L46-72）
- `packages/coding-agent/src/core/agent-session.ts`：guidelines 聚合（L1026-1054）
- 机制细节回指：`../../agent/04-Harness/4.3_System_Prompt.md`

## 最小例证是什么

**例证 1（文档地图 vs 全文）：** 读 `system-prompt.ts:131-138`——它给的是 `docs/extensions.md` 这种**路径 + 主题映射**，不是 extensions.md 的正文。证明"教 agent 查"而非"替 agent 背"。
