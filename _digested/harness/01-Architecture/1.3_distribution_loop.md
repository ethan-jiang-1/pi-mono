...
`package.json` 的 `pi` 字段只有四个 string 数组（`extensions`/`skills`/`prompts`/`themes`），由 `readPiManifest`（`core/pi-manifest.ts:16`）解析。没有 manifest 时回退到 `index.ts`/`index.js`。
...
- `packages/coding-agent/src/core/pi-manifest.ts`：`PiManifest`（L3）、`readPiManifest`（L16）——4 字段的资源声明
- `packages/coding-agent/src/core/package-manager.ts`：`pi install`/`remove`/`list`/`update` 的包管理入口（`install` L801 附近、scope 收集 L890-1055）
- `packages/coding-agent/src/core/extensions/loader.ts`：`resolveExtensionEntries`（L610）——manifest → 入口路径
