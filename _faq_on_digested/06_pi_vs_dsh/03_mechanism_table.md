...
> - pi：`_digested/harness/01-Architecture/{1.1,1.2,1.3,1.4}`、`_digested/harness/02-Boundaries/`、`_digested/extensions/`（`01-Core/1.2`、`02-Expansion/`）、`_digested/agent/01-Anatomy/1.4`（源码或引用统一以 `v0.84.4` / `b79e4cc83` 为基线）。
...
| **核心形态** | 稳定核心（loop + 8 工具 + 统一扩展口） | 组合内核（Cordis 五原语为基础；`new Context()` 硬编码根，Loader 由 `boot()` 安装） |
...
| **默认值** | 高（8 工具，可再装扩展） | 默认由 bundle/profile 组合提供（`dsh-base` 为每 profile 打底层），不内置单一工具集 |
