...
pi 的第三方能力注入只有**一个口子**：`ExtensionAPI`。tool、slash command、键盘快捷键、CLI flag、provider、system prompt 修改、自定义 UI、消息注入、扩展间通信，全部挂在同一个 `pi` 对象上（`extensions/types.ts:1232`），走同一套 jiti 加载 + 事件扇出 + 生命周期。
...
- `packages/coding-agent/src/core/extensions/types.ts`：`ExtensionAPI`（L1232）——唯一注入口的完整形态；`UserBashEventResult.operations`（L1119）——外层落到内层的证据
