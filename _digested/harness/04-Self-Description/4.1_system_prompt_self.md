...
`Available tools` 只列"提供了 snippet 的工具"（`system-prompt.ts:80-130`），guidelines 从工具的 `promptGuidelines` 聚合（`agent-session.ts:1070-1081`）。所以工具集变化时，prompt 里的工具列表和准则**自动跟着变**。
