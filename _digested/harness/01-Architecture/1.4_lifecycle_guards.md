...
普通 `emit*()` 都 try-catch 隔离 handler 异常；唯独 `emitToolCall`（`runner.ts:932`）**故意不加 try-catch**，异常一路传到 `agent-session.ts:484` 的 `_installAgentToolHooks`，该 tool call 以错误收场。
...
- `packages/coding-agent/src/core/agent-session.ts`：`_installAgentToolHooks`（L484）——emitToolCall 异常的 fail-close 传播路径
