...
> **当前源码基线**：pi-mono `v0.84.4`（tag `b79e4cc83`，**已 merge 进本仓库**——merge `f9a1cf489`，源码锚点对应工作树）。2026-09-01 完成第四轮 catch-up（v0.84.3→v0.84.4，41 commits，fix-heavy 维护版本）：agent-loop `prepareNextTurn` 时序 breaking 重排（between-turn 阈值压缩，#8782）、compaction `toolChoice` 撤回 + 截断 summary 拒绝、RPC `clear_queue`、扩展 `ui_prompt_*` 事件（`on()` 重载 33 个）、ai 层流式稳健性修复；重锚 2.1/2.4/2.6/3.1/3.2/1.4/4.4 等篇行号，并更正上轮 `on()` 重载计数笔误（34→实为 31，现 33）。行号复核仍不完全——以各篇警示为准。版本演进和旧行为只记录在 [`_change_log/`](./_change_log/README.md)。
>
