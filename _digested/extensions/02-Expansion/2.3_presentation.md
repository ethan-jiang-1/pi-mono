...
**"显示"和"行为"必须能分开改。** 如果改一个工具的长相就必须重写它的执行逻辑，那"只想要更紧凑的显示"就要承担"重新实现 read"的全部风险。pi 的答案是：**工具的执行 override 和渲染 override 是两个独立的 slot**——改长相不必重写行为，改行为也不必重写长相。
...
这解释了为什么 7 个内置工具必须是"可复用工厂"（`createReadTool` 等，见 [1.1](../01-Core/1.1_minimal_core.md) 判断二）：**如果工具是黑盒，长相就改不了。**
...
也就是说，覆盖执行时，渲染自动回落到内置；覆盖渲染时，执行自动回落到内置。**两个 slot 各改各的，缺省继承。** 这让"包装内置工具做日志/访问控制"（`tool-override.ts`）和"只改显示风格"（`minimal-mode.ts`）成为两个正交、可独立做的动作，而不是一个"必须全量重写"的动作。
...
这说明**"长相"接缝不是纯装饰**：它把一些**只对 UI 有意义、不对 LLM 有意义**的数据（git branch、token 统计）暴露给了扩展，让扩展能自建状态栏，而不需要核为每个可能的 footer 元素开一个 API。核只给一个 `setFooter(renderFn)` + 一个 `footerData` 钩子，footer 长什么样是扩展的事。

### 判断四：编辑器、overlay 是"长相"轴的深水区（硬事实 + 解释）
...
2. **执行和渲染正交，降低了"包装内置工具"的风险**（硬事实）。`tool-override.ts` 能只加审计不改 UI，`minimal-mode.ts` 能只改 UI 不碰执行。两件最常被"包装"的需求，都不需要重写工具逻辑。
