...
| package README | 用/改某包的人或 agent | **API 参考**（如 `packages/agent/README.md` 517 行：Quick Start / Event Flow / Options / Methods） | 不是"合同"，是教程式参考 |

**关键观察**：`README.md:51` 写的是 AGENTS.md *"for both humans and agents"*——Pi 把人和模型混为一谈，没有 DSH 的两入口分流。
...
| `verify-md-links` / `verify-doc-budgets` 机器检查 | 无（`scripts/` 里只有 build/release/lockfile/model-catalog，v0.85.1 又加了 entry-graph 与 runtime-deps 守卫，但**都不是**文档链接/字数门禁） |
| `CLAUDE.md` symlink → `AGENTS.md` | 无 CLAUDE.md |

**一个具体后果**：`README.md` 的 "All Packages" 表只列了 6 个包（chord/telemetry/ai/agent/coding-agent/tui），漏掉 experimental 的 protocol/client/server/evals 和 infra 的 session-backends。没有机器检查，这种"地图漂移"不会被 CI 发现——这正是"静态层不保证地图不坏"的直接证据。（`chord` 是 v0.85.1 才补进表的；`git diff v0.84.4..v0.85.1 -- README.md` 恰好只有这一行插入，说明补一个包是"想起来才补"，不是有守卫驱动。）
...
2. **package README 定位成"API 参考"而非"合同"**：`agent/README.md` 517 行把事件序列、选项、方法都写全了——这是"给 SDK 使用者的参考"，不是 DSH 式的"短合同"。两种定位各有取舍。
...
- `README.md`：L11（auto-close 提示）、L24（"ask the agent to explain itself"）、L26-35（All Packages 表，只列 6 个）、L51（AGENTS.md "for both humans and agents"）
- `AGENTS.md`（根）：169 行，无 repository layout 节、无 core-minimal 总模型
- `CONTRIBUTING.md`：开篇 "pi's core is minimal" + "The One Rule"
- `packages/agent/README.md`：517 行 API 参考（Quick Start / Event Flow / Options / Methods）
