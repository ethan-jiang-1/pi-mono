...
1. **extension**（`ExtensionAPI`）：`export default (pi) => { pi.on(...); pi.registerTool(...); }` —— 33 个 `on()` 事件重载 + 约 20 个方法（`extensions/types.ts:1252` 起，重载列表 L1257-1301；v0.84.4 新增 `ui_prompt_start`/`ui_prompt_end`。注：v0.84.3 轮曾记 "34 个"，实测当时为 31，属笔误，本轮更正）。
