...
普通 `emit*()` 都 try-catch 隔离 handler 异常；唯独 `emitToolCall`（`runner.ts:1035`，v0.86.1 重锚）**故意不加 try-catch**，异常一路传到 `agent-session.ts:507` 的 `_installAgentToolHooks`，该 tool call 以错误收场。
...
**⚠️ v0.84.3 新增第四道护栏（失败 factory 状态丢弃，#8424）：** `createExtensionAPI` 返回 `{api, commit, discard}`（`extensions/loader.ts:259`）。加载期注册是 `pendingFlagValues`/`pendingRuntimeChanges`/`loadingUnsubscribers` 的暂态，`commit()` 成功才生效；factory 失败时 `discard()` 标记状态为 failed、退订 bus、清空 pending，之后任何 API 访问都抛 "failed to load and its API is no longer active"（loader.ts:266/473）。这与 stale 保护同一哲学：**宁可让已死扩展的访问当场炸，也不让它留下半初始化状态继续被调用。**
...
- `packages/coding-agent/src/core/extensions/runner.ts`：`bindCore`（L317）、`invalidate`（L593）、`assertActive`（L602）、`emitToolCall`（L982，无 try-catch 的例外）
- `packages/coding-agent/src/core/agent-session.ts`：`_installAgentToolHooks`（L486）——emitToolCall 异常的 fail-close 传播路径
