# 1.2 故意不做：plan-mode 和 subagent 被"赶"成了示例
...
核心拉扯是：**pi 该不该替用户决定"工作流应该长什么样"？** 极简核的答案是"不"——核只提供拼工作流的积木，不提供拼好的工作流。
...
### 判断二：subagent 是"隔离上下文"被做成文件 + 提示词（硬事实 + 解释）
...
pi 不内置 plan-mode/subagent，不是因为它们不重要，而是因为它们的**价值高度依赖用户的偏好**：
