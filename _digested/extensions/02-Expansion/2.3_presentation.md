...
`minimal-mode.ts`（426 行）覆盖全部 8 个内置工具，但每一个都是同一个套路：
...
**关键：** 执行用 `getBuiltInTools(ctx.cwd).read.execute` 原样转发（`getBuiltInTools` 是文件内把 `createReadTool(cwd)` 等 8 个工厂打包的本地封装），只重写 `renderCall`/`renderResult`。整个"minimal mode"（折叠时只显示工具名、不显示输出）就是**纯渲染层的改动**，执行路径一行没碰。
