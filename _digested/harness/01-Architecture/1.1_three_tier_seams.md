...
1. **extension**（`ExtensionAPI`）：`export default (pi) => { pi.on(...); pi.registerTool(...); }` —— 32 个 `on()` 事件重载 + 约 20 个方法（`extensions/types.ts:1198-1437`）。
2. **tool factory**（`createReadTool` / `createBashTool` 等）：接收 `options`（其中含 `operations` 和 `promptSnippet` 来源），返回 `AgentTool`（`tools/read.ts:209`、`tools/bash.ts:322`）。
3. **operations**（`ReadOperations` / `BashOperations` / `EditOperations` 等）：每个工具的一个小接口，比如 `BashOperations.exec(command, cwd, {onData, signal, timeout, env})`（`tools/bash.ts:62`）。

三层是**复用下坠**：extension 的 `user_bash` 事件能返回一个自定义 `BashOperations`（`extensions/types.ts:1083`），把最外层能力一路落到最内层。
...
| extension | **行为组合**（命令/事件/provider/UI/消息） | 极大（32 事件 + ~20 方法） | 生命周期、事件扇出、两阶段绑定、失效保护 |
...
- 接口极小：`exec(command, cwd, {onData, signal, timeout, env}) => {exitCode}`（`bash.ts:62`）。调用方只学一个方法。
- 实现极深：`createLocalBashOperations`（`bash.ts:88`）背后是 shell 选择（`getShellConfig`）、stdin/args 两种命令传输、`detached` 进程组、流式 `onData`、超时解析（`resolveTimeoutMs`）、cwd 存在性校验。
...
`ExtensionAPI` 有 32 个 `on()` 重载 + ~20 个方法，**不是深模块**——用 Ousterhout 的标准它是"大接口"。但这里大是**对的**：
...
- `packages/coding-agent/src/core/extensions/types.ts`：`ExtensionAPI`（L1198-1437，32 个 `on` 重载 + 方法族）、`UserBashEventResult.operations`（L1083，外层落到内层的证明）
...
**例证 3（三层的接口大小梯度）：** 直接看三个接口的成员数——`BashOperations` 1 个方法，`ToolDefinition` ~8 字段，`ExtensionAPI` 32 事件 + ~20 方法。梯度是"调用频率递减、一次性学习递增"的映射，不是随意。
