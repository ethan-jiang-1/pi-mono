...
> **警示（v0.85.x–0.86.x）**：事件面仍在按核内新能力同步扩轴——如 cache warming 引入的 `cache_warming_decision`（`cache-warmer.ts:114`，扩展可在刷新前 `"stop"` 或强制 refresh，`extensions/types.ts:1295`）。"核加一点内部能力，就同步开一条事件接缝"的惯例未变，且新轴仍走同一条 `pi.on()`。
