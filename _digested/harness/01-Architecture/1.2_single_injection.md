...
pi 的第三方能力注入只有**一个口子**：`ExtensionAPI`。tool、slash command、键盘快捷键、CLI flag、provider、system prompt 修改、自定义 UI、消息注入、扩展间通信，全部挂在同一个 `pi` 对象上（`extensions/types.ts:1252`），走同一套 jiti 加载 + 事件扇出 + 生命周期。
...
- `packages/coding-agent/src/core/extensions/types.ts`：`ExtensionAPI`（L1252）——唯一注入口的完整形态；`UserBashEventResult.operations`（L1139）——外层落到内层的证据
- `packages/coding-agent/src/core/extensions/runner.ts`：`bindCore`（L317）、`invalidate`（L593）——写一次、覆盖全部能力类型的护栏
...
**例证 3（锁定的实锤）：** grep `packages/coding-agent/src` 和 `packages/agent/src` 找 `mcp` / `acp`——只有 vendored `highlight.min.js`、图片工具与 `interactive-mode.ts` 的子串误匹配，没有实现。证明统一口子确实没留 MCP/ACP 的旁路（锁定是真实的，见 [2.2](../02-Boundaries/2.2_no_interop_protocol.md)）。
