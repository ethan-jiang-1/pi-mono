...
3. **回收（recycle）**：超预算由 compaction 压缩回收，skills 被 read 工具闸住、无 read 时整块不生成（不给空承诺）。
...
- **docs 正文**：文档地图（`system-prompt.ts:131-138`）写了 "read only when the user asks about pi itself"，并给逐主题路径。模型被问 pi 自身时才 read `docs/extensions.md` 等——docs 随包（04 的 `files`），read 读的是本机文件。
- **skill 正文**：`<available_skills>` 只给 name/description/location 目录，模型决定用时 `read` 读 `SKILL.md` 全文（机制见 [`_digested/agent/04-Harness/4.2_Skills.md`](../../_digested/agent/04-Harness/4.2_Skills.md)）。
...
| `SKILL.md` 在 skills 目录 | `<available_skills>` 目录 + read 闸门（拉） |
