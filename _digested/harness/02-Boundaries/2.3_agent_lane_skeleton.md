...
> **⚠️ v0.86.1 时效警示（2026-09-21，见 `_change_log/0006-v0.84.4-to-v0.86.1.md`）**：本文的现象描述基于 **v0.84.2**，作为 v0.85.x–0.86.x 之前的历史断面仍然成立，但核心结论已过时：WP00–WP09（`packages/agent/docs/work-packages/`）已把骨架填成完整的 **durable drive 运行时**——`HarnessNotImplemented` 已删除（被 `SliceNotImplemented` 取代，仅剩个别 slice），`prompt`/`skill`/`compact`/`abort` 等操作均为持久化、可恢复的真实实现（`runtime/harness.ts:29`、`runtime/lane.ts:221`、`runtime/drive.ts:29`，机制详见 [`agent/04-Harness/4.1`](../../agent/04-Harness/4.1_AgentHarness.md)）。"library-first 的内层 harness 只有契约没有实现"这句评价在 v0.86.1 已不再成立；本文保留作历史评价与 `AgentSession`/runtime 双实现并存的背景。
...
**用 [1.1](../01-Architecture/1.1_three_tier_seams.md) 的词汇**：这是一个**设计好的 seam，但真正的 adapter 还没成型**。注意"谁消费 AgentLane"这个检验：`AgentSession` 并**不**消费它——SDK 是直接在 `Agent` 类（`agent.ts`）之上组装的，走的是另一条路。AgentLane 唯一的代码级 consumer 是 `packages/coding-agent/src/server/create-harness.ts` 的 `createCodingAgentHarness()`（它会 `AgentHarness.create()` 并构造出 harness），但这个 consumer 自身也是 stub——它拿到的 harness 的 `prompt`/`skill`/`compact` 等操作全部 `Reject HarnessNotImplemented`。（**v0.86.1 注**：该文件已删除，函数移至 `experimental/session-worker.ts:805`；stub 前提同样已被 durable drive 推翻，见顶部警示。）所以准确说法是：**AgentLane 是"唯一的 consumer 也是半成品"的假想 seam**，而不是"一个半 adapter"（后者会让人误以为 AgentSession 是它成熟的一侧）。
...
- `packages/coding-agent/src/server/create-harness.ts`：`createCodingAgentHarness`——代码里唯一构造 `AgentHarness` 的地方（L153 调 `AgentHarness.create`），但其 harness 的操作是 stub（**v0.86.1 注**：`create-harness.ts` 已删除，`createCodingAgentHarness` 移至 `packages/coding-agent/src/experimental/session-worker.ts:805`；且随 4.1 所述 durable drive 落地，其 harness 操作不再是 stub）
...
**例证 3（seam 的 consumer 也是 stub）：** 用 [1.1](../01-Architecture/1.1_three_tier_seams.md) 的真 seam 检验——"谁消费 AgentLane 这个 seam"？不是 `AgentSession`（它直连 `Agent` 类）；代码里唯一构造 `AgentHarness` 的是 `createCodingAgentHarness()`（v0.84.2 时在 `coding-agent/src/server/create-harness.ts:153` 调 `AgentHarness.create()`；**v0.86.1 移至 `experimental/session-worker.ts:805`**），而这个 harness 的操作曾是 stub。一个"其唯一 consumer 也是半成品"的 seam，比"一个半 adapter"更接近 v0.84.2 现状—— durable drive 落地后此例证不再成立（见顶部警示）。
