...
| agent 启动前 | `before_agent_start` | 注入（inject）消息、强制改写 system prompt（`systemPrompt`，请求级投影） |
...
![循环接缝：沿 agent loop 的拦截点（30+ 事件归纳成 9 个动作点）](figures/2.1_loop_seams.svg)
...
> **警示（v0.85.x–0.86.x，#9548 transcript-first 重构）**：`before_agent_start` 现在可返回 `systemPrompt` 做整体覆盖（`BeforeAgentStartEventResult.systemPrompt`，`extensions/types.ts:1167-1171`），且语义分两路——**普通** prompt/tool 变更经 `_preparePromptAndToolLoadout()`（`agent-session.ts:1149`）与 `buildSystemPromptSections` 做 section diff，产出一条 `SystemMessage` patch（`sections` 按名 patch、`toolsAdded`/`toolsRemoved`）**持久化进 session transcript**；而 **forced prompt**（handler 返回的 `systemPrompt`）走 `_installAgentForcedPromptProjection()`（`agent-session.ts:1166-1190`），经 `transformContext` **投影为请求头部的单条 system message——发送但不落盘**（transcript 保持结构化 sections）。旧描述"改写 system prompt = 追加进 prompt"的心智模型已失效：持久化与"发送给 LLM"现在是两个正交动作。
