...
内置工具只有 8 个（`tools/index.ts` 的 `ToolName` 联合：read/bash/edit/write/grep/find/ls/powershell，v0.84.3 起含 Windows-only 的 powershell），而 `examples/extensions/` 有 70+ 个能力（todo、permission-gate、git-checkpoint、custom-provider、plan-mode、subagent……）全部做成了扩展。

**比例本身就是证据**：如果 core 不 minimal，这 70 个里会有一半被塞进内置工具。8 vs 70+ 不是偶然，是"不许进 core"执行了若干轮之后的结果。
...
### 判断五（v0.85.1 新增物证）："入口即成本契约"把这条纪律从价值判断推进到**可量化预算**（硬事实）

core-minimal 一直有"功能要不要放进来"的闸；v0.85.1 又加了一道更细的：**包入口的模块图是有预算的**。`scripts/check-entry-graphs.mjs` 把每个 `exports` 入口的**价值导入图**走一遍，按 `BUDGETS` 表逐条强制 `maxFiles` 与 `forbid` 黑名单。文件头注释原文：

> "Entry points are cost contracts. ... one stray `export *` can silently make a narrow entry drag an entire barrel: importing a 1-file pure function through a barrel costs ~37 MB of evaluated module graph, and nothing fails until someone measures a process."

**为什么这是比"8 vs 70+"更强的证据**：它把"core 要小"从**价值判断**变成**可量化、可 CI 强制的预算**。样本（`scripts/check-entry-graphs.mjs:33-41`）：

| 入口 | 预算 |
|---|---|
| `packages/ai` `./utils/*` | `{maxFiles: 3, forbid:["providers/","api/","index.ts"]}` |
| `packages/agent` `./harness/context` | `{maxFiles: 6, forbid:["harness/runtime/","harness/execution/","packages/ai/"]}` |
| `packages/agent` `./harness/session` | `{maxFiles: 25, forbid:["harness/runtime/","harness/execution/","packages/ai/src/index.ts"]}` |

规则本身也很"core-minimal"：`.` 与 `./node` 是 batteries-included 入口、不设上限，**每一个窄入口都必须声明自己允许触达的图**（`:31-32`）。这和"能力被逼到边界外"是同一条纪律，只是这次量的是**字节**而不是功能个数。

**联动**：这条守卫已进 `npm run check` 链（见 [3.4](./3.4_engineering_rules.md)）；coding-agent 侧 `core/tools/renderers/` 的拆分（[1.1](../01-Architecture/1.1_three_tier_seams.md) 判断六）是同一约束的落地——17 MB 与 37 MB 是同一套语言。
...
- `packages/coding-agent/src/core/tools/index.ts`：`ToolName` 联合（L95，v0.84.3）——内置工具只有 8 个
- `packages/coding-agent/examples/extensions/`：70+ 个扩展能力（对比 8 个内置）
- `scripts/check-entry-graphs.mjs`：`BUDGETS`（L33，逐入口 `maxFiles`+`forbid`）、文件头 "Entry points are cost contracts"——纪律被量化成预算（判断五）
- 因果链的另一端回指：`../01-Architecture/1.1_three_tier_seams.md`、`../01-Architecture/1.4_lifecycle_guards.md`
- 同一事实的扩充视角回指：`../../extensions/01-Core/1.1_minimal_core.md`（核的具体清单与"进核 vs 扩展"判据，与本节互补）

## 最小例证是什么

**例证 1（比例物证）：** 数 `tools/index.ts` 的 `ToolName` 联合 = 8 个；数 `examples/extensions/` 顶层 `.ts` 文件 + 子目录 = 70+ 个能力。8 vs 70+ 的比例就是"能力被逼到扩展面"的直接证据。
