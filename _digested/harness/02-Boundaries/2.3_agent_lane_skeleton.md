...
- `packages/agent/src/harness/agent-harness.ts`：`ActionInfo`（L182）、`HookName`（L198）、`Hooks`/`Events`（L211/L215）、`UnavailableRegistry`（L219，hook 注册抛 `HarnessNotImplemented`）、`AgentHarnessOptions`（L243）、`HarnessTool`（L237）、`HarnessNotImplemented`（L74）
