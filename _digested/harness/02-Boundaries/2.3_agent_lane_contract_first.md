...
4. **连带作废一条 0003 时代的旧结论。** 0003 记录里那条"官方 `harness.md` 描述的是更超前的 registers/SQLite 目标模型、与已发布代码不是一套 API"**已失效**（`register` 在 spec 里只剩 3 次英文动词；spec 自带 §0.9 implementation-status 清单；spec 与代码同批 commit 一起改）。本篇判断一、三引用的 `harness.md` 现在是**实现规格**，不是目标态文档——完整反转见 [`../README.md`](../README.md) 的"旧结论反转"一节。
