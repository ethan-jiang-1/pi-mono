...
1. **extension**（`ExtensionAPI`）：`export default (pi) => { pi.on(...); pi.registerTool(...); }` —— 37 个 `on()` 事件重载 + 约 20 个方法（`extensions/types.ts:1266` 起，重载列表 L1266-1326；v0.86.1 新增 `cache_warming_decision` 且全部重载返回 unsubscribe（#8967）；v0.84.4 曾为 36 个（含 `ui_prompt_start`/`ui_prompt_end`）。注：重载列表含 3 个多行格式——`session_before_switch`/`session_before_compact`/`before_provider_request`——单行 grep 口径会少计 3，2026-09-01 二次复核更正）。
2. **tool factory**（`createReadTool` / `createBashTool` 等）：接收 `options`（其中含 `operations` 和 `promptSnippet` 来源），返回 `AgentTool`（`tools/read.ts:195`、`tools/bash.ts:400`，v0.86.1 重锚；定义工厂分别在 read.ts:64 / bash.ts:393）。
...
三层是**复用下坠**：extension 的 `user_bash` 事件能返回一个自定义 `BashOperations`（`extensions/types.ts:1144`，v0.86.1 重锚），把最外层能力一路落到最内层。
...
`createReadToolDefinition(cwd, options)` 返回的 `ToolDefinition` 同时带着 `promptSnippet` 和 `promptGuidelines`（`read.ts:74-75`，v0.86.1 重锚；旧 :219-220 已漂移），再由 `wrapToolDefinition`（`tool-definition-wrapper.ts:5`）包成 `AgentTool`。
...
`ExtensionAPI` 有 33 个 `on()` 重载 + ~20 个方法，**不是深模块**——用 Ousterhout 的标准它是"大接口"。但这里大是**对的**：
...
- `packages/coding-agent/src/core/tools/bash.ts`：`BashOperations`（L62）、`createLocalBashOperations`（L158）、`createBashToolDefinition`（L529，`options.operations` + `promptSnippet`）
...
- `packages/coding-agent/src/core/tools/edit.ts`：`EditOperations`（L96）、`defaultEditOperations`（L105）
- `packages/coding-agent/src/core/tools/tool-definition-wrapper.ts`：`wrapToolDefinition`（L5）——tool 层到外层运行时的桥
- `packages/coding-agent/src/core/extensions/types.ts`：`ExtensionAPI`（L1252，33 个 `on` 重载 + 方法族）、`UserBashEventResult.operations`（L1139，外层落到内层的证明）
