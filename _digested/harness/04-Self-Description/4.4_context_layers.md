...
1. **harness 自述**（system prompt）：你是谁、在 pi 里、有这些工具、pi 文档在哪——`system-prompt.ts:121-138`。
2. **项目约定**（`<project_context>` 块）：当前项目自己的 `AGENTS.md`/`CLAUDE.md`——`system-prompt.ts:145-152`。
3. **领域知识**（`<available_skills>` + read 按需取）：`SKILL.md` 的领域指令——`system-prompt.ts:155`。

项目约定的发现逻辑在 `resource-loader.ts`：`loadProjectContextFiles`（L118）从 cwd 向上遍历到根，收集 `AGENTS.override.md` / `AGENTS.md` / `CLAUDE.md`（`loadContextFileFromDir` L70-71 的候选表），加上全局 agentDir 的一份，去重、处理 worktree shadow 后返回。
...
`buildSystemPrompt` 的结构是顺序追加：身份+工具+文档地图 → `<project_context>`（`system-prompt.ts:145`）→ `<available_skills>`（`system-prompt.ts:155`）→ cwd。

**深意**：分层是**结构化**的——`<project_context>` 包一层，其内每个 `<project_instructions path="...">` 子元素带 `path` 属性（`coding-agent/src/core/system-prompt.ts:146-151`）、`<available_skills>` 有 name/description/location 字段。agent 读到的是"三份可区分的结构化信息"，不是"一段混着 harness 说明、项目规则、领域知识的散文"。这对应 [04-README](../04-Self-Description/README.md) 的"分层"判据。
...
skills 不进 prompt 正文，只进 `<available_skills>` 目录；agent 需要时用 read 工具读 `SKILL.md` 全文。且 skills 块只在 read 工具可用时才生成（`system-prompt.ts:155`）。
...
- `packages/coding-agent/src/core/resource-loader.ts`：`loadContextFileFromDir`（L70，候选表）、`loadProjectContextFiles`（L118，向上遍历 + 去重 + worktree shadow）、`findShadowedContextFile`（L100）
- `packages/coding-agent/src/core/system-prompt.ts`：`<project_context>` 追加（L145-152）、`<available_skills>` 闸门（L155）
- `packages/coding-agent/src/core/skills.ts`：`formatSkillsForPrompt`（L335）——skills 目录的生成
