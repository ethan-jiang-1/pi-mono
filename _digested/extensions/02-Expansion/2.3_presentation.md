...
`minimal-mode.ts`（426 行）覆盖 7 个跨平台内置工具（`getBuiltInTools` 本地封装不含 Windows-only 的 powershell），但每一个都是同一个套路：
...
**关键：** 执行用 `getBuiltInTools(ctx.cwd).read.execute` 原样转发（`getBuiltInTools` 是文件内把 `createReadTool(cwd)` 等 7 个工厂打包的本地封装），只重写 `renderCall`/`renderResult`。整个"minimal mode"（折叠时只显示工具名、不显示输出）就是**纯渲染层的改动**，执行路径一行没碰。

这解释了为什么 8 个内置工具必须是"可复用工厂（factory）"（`createReadTool` 等，见 [1.1](../01-Core/1.1_minimal_core.md) 判断二）：**如果工具是黑盒（black box），长相就改不了。**
