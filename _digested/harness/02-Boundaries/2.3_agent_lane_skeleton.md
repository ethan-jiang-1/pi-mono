...
**用 [1.1](../01-Architecture/1.1_three_tier_seams.md) 的词汇**：这是一个**设计好的 seam，但只有一个半 adapter**——`packages/coding-agent` 的 `AgentSession` 是实际消费 agent 包运行时的那个 adapter（成熟），而"外部宿主直接用 AgentLane 自组运行时"这第二个 adapter 还没成型。一个 adapter 是假想 seam，两个是真 seam——**AgentLane 目前处在一个半的尴尬位置**。
...
- `packages/agent/src/harness/agent-harness.ts`：`ActionInfo`（L182）、`HookName`（L198）、`Hooks`/`Events`（L211/L215）、`UnavailableRegistry`（L223，hook 注册抛 `HarnessNotImplemented`）、`AgentHarnessOptions`（L240）、`HarnessTool`（L247）、`HarnessNotImplemented`（L74）
- `packages/agent/src/agent.ts`：`Agent` 类——agent 包当前可用的运行时核心
...
**例证 1（契约完整 + 实现骨架）：** 数 `ActionInfo` 的 member（14 种 kind）和 `HookName`（11 个）——契约画得很细；再看 `UnavailableRegistry.on()`（`L223`）——任何 hook 注册都 `throw HarnessNotImplemented`。同一个文件里"契约的丰富"和"实现的空"并存。
...
**例证 3（半 adapter 状态）：** 用 [1.1](../01-Architecture/1.1_three_tier_seams.md) 的真 seam 检验——"谁消费 AgentLane 这个 seam"？只有 coding-agent 的 `AgentSession` 一侧成熟，没有第二个"外部宿主直用"的 adapter。一个半 adapter = 假想 seam 和真 seam 之间。
