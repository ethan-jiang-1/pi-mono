...
pi 有**局部的**兼容处理，但不是契约：`loader.ts` 把 `@earendil-works/pi-ai` 解析到 compat 入口（"strict superset of the old global API... until compat is removed"，`loader.ts:59-64`），`types.ts` 里有 `@deprecated` 字段标"Retained for source compatibility"（`types.ts:1518`）。这些是**点状的 shim**，覆盖 pi-ai 的旧 API，不覆盖 `ExtensionAPI` 的整体演进。
...
- `packages/coding-agent/src/core/extensions/types.ts`：`@deprecated`（L1518）——枚举式兼容标记
