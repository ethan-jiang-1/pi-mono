...
## 社区验证的增量做法(2026-09 深挖,细节见 [06 篇](06_field_usage.md))

上面的标准动作经三路外部证据复核后,补充六个被多人验证的增量:

1. **讨论优先模式**(场景 A):把 pi 的系统提示换成 "thinking partner——默认只讨论,不动文件不跑命令,除非明确要求"。pi 的 ~500 token 极简提示让你的规则真正有约束力(CC 要跟 ~20k token 的内置产品逻辑竞争)。
2. **`.scratch/` + `n2c:` 标注回路**(场景 A 的 A2 升级):计划写进 gitignored 的 `.scratch/plans/`,人逐行标注 `n2c:`(note to Claude),agent 逐条回应后再执行——"计划落盘"的人审环节有了具体机制。
3. **上下文交接点**(全场景):100–150k token 就做 `/continue` 式交接(总结进文件、新会话载入),≤200k 硬顶;每个 skill/扩展都有 token 价,装前审计。
4. **自扩展优先**(A4/B5 升级):需要新能力时先"让 pi 给自己写扩展/skill"(Armin:头号 idiom;他连浏览器自动化都是自造 CDP skill),下载只给第三遍才用到的东西。
5. **信息加工 = 小脚本流水线**(场景 B 的方法论级答案,Mario 的实证案例):18,000 行数据不是"让 agent 一次处理完",而是让它搭一条 file-in/file-out 的小脚本链(转换→拆分→过滤→分析→统计→可视化);领域专家逐段验证**产出**而非代码,上游改错后重跑对应段即可——可复现性即质量门。这与你 `_handbook_for_information_work` 的"可审查交付物"完全同构。
6. **tmux 作编排层**(场景 C):pi-agent-hub(多会话驾驶舱 + worktree 管理)、pi-tmux(`/supervise` 监督者模式,主 agent 不自知被监督);运维场景 pi-hosts 实测省 4 倍时间与一半 token。

另外一条反向提醒(来自作者本人与 HN 高赞):**DX > AGENTS.md 长散文**——把类型检查/lint 做成 commit hook 强制 agent 自己修,比把诊断或规则堆进上下文更有效(场景 A0 第 3 条的强化版)。
